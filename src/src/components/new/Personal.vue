    ...
    <section class="content">
      ...
      <ul class="list2">
        <li class="item clearfix">
          ...
          <router-link to=""><span class="right">{{userInfo.showName}}<i class="more"></i></span></router-link>
        </li>
        <li class="item clearfix">
...
<script>
  ...
  import {minimg} from '../../assets/js/compress.img.js'
  import {oCommon} from '../../assets/js/common'
  let self;
  ...
  export default {
    ...
    mounted() {
      self = this;
      oCommon.get("/api/my/info",(response) => {
    ...
    methods: {
      ...
      selectSex(value) {
        ...
        oCommon.post("/api/my/editInfo",{
          ...
          if (response.data.result === 1) {
            ...
            oCommon.toast(response.data.msg);
          }
        })
      },
      /**
        ...
        MessageBox.prompt('请输入昵称').then(({value, action}) => {
          if (value) {
            oCommon.post("/api/my/editInfo",{
              ...
              if (response.data.result === 1 && value.length <= 10) {
                oCommon.toast("修改成功");
      ...
      imgChange(e){
        ...
        reader.onloadend = function () {
          let img = this.result;
          oCommon.post("/api/upload/avatar",{
            "data": img
          },(response) => {
            if(response.data.result === 1) {
              let imgName = response.data.data.name;
              let imgSrc = response.data.data.src;
              oCommon.post("/api/my/editInfo",{
                "headerImg": imgName,
                "nickname": self.userInfo.nickname
              },(response) => {
                if(response.data.result === 1) {
                  oCommon.toast("头像修改成功");
